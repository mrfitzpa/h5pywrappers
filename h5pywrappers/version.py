...
def _get_version_summary():
    """Get version summary of ``h5pywrappers``.
    ...
    -------
    summary : `str`
        Version summary of ``h5pywrappers``.
    """
    # Check versions of h5pywrappers.
    from . import _version
    if _version.version != version:
        raise ValueError("h5pywrappers version has changed since "
                         "installation/compilation")



    # Generate summary.
    summary = ("h5pywrappers {h5pywrappers_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}")
    
    summary = summary.format(h5pywrappers_ver=full_version,
